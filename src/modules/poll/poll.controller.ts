...
export class PollController {
  ...
  createPoll = async (req: Request, res: Response) => {
    ...
    const result = await this.pollService.createPoll({ input, user: req.user });
  ...
  listPolls = async (req: Request, res: Response) => {
    ...
    const result = await this.pollService.listPolls({ query, user: req.user });
  ...
  getPoll = async (req: Request, res: Response) => {
    ...
    const result = await this.pollService.getPoll({ pollId: params.pollId, user: req.user });
    return res.status(200).json(result);
  };
  ...
  updatePoll = async (req: Request, res: Response) => {
    ...
    const result = await this.pollService.updatePoll({
  ...
  deletePoll = async (req: Request, res: Response) => {
    const params = pollIdParamSchema.parse(req.params); // 경로 파라미터 검증
    const result = await this.pollService.deletePoll({ pollId: params.pollId, user: req.user });
    return res.status(200).json(result);
  };
}
